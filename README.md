<h1 align="center">Hey, I'm Shiv</h1>

<p align="center">
<a href="https://shivcdhry.live?utm_source=githubprofile">portfolio</a> •
<a href="https://linkedin.com/in/shiv-ratan-choudhary">linkedin</a> •
<a href="https://www.shivcdhry.live/blog">blog</a> •
<a href="mailto:shivbera45@gmail.com">email</a>
</p>

- 🔭 backend engineering @ [Regie.ai](https://regie.ai) — building & shipping ai for sales (prev @ BigBrick)
- 🌱 into systems, search infra & applied ai
- 👨‍💻 building & shipping at [shivcdhry.live](https://shivcdhry.live?utm_source=githubprofile) — checkout my work there
- 📝 i write about databases & system design at [shivcdhry.live/blog](https://www.shivcdhry.live/blog)
- 🛠️ `langs: c++ • go • python • typescript` `web: react/next • bun/node • fastapi • postgres • mongo • redis • kafka` `ops: docker • git • prometheus • grafana`

- 💬 ask me about `system design • search • backend • cp`
- 📫 reach me at **shivbera45@gmail.com**
- ⚡ fun fact: 5★ CodeChef [2040](https://www.codechef.com/users/fast_sand_38) • State Rank 1 (Class 12, 100%)

### selected work
- [artemus](https://github.com/shivbera18/quiz-repo) — ai banking exam platform · [live](https://artimusquiz.vercel.app)
- [matrix-mul](https://github.com/shivbera/matrix-mul) — strassen/winograd + deepmind optimization
- [style-transfer](https://github.com/shivbera18/arbitrary-style-transfer) — in-browser tf.js transfer · [live](https://arbitrary-style-transfer.vercel.app)
- [clone-detector](https://github.com/shivbera18/CloneCodeDetection) — siamese bilstm, 50m params
- [rubiks-solver](https://github.com/shivbera18/Rubiks-Cube-Solver) — 3×3 + IDA*

### writings
- [Why Instacart Ditched Elasticsearch for Postgres](https://www.shivcdhry.live/blog/elasticsearch-vs-postgres-search)
- [Choosing Databases Like a Senior Engineer](https://www.shivcdhry.live/blog/pick-the-right-database)
- [Poverty & Women Employment Correlation](https://www.shivcdhry.live/blog/economics-poverty-women-employment-correlation)




